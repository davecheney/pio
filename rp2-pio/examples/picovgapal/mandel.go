...
func newRenderer(fb *picovga.Framebuffer, maxIter int) *renderer {
...
// draw renders the whole picture, left to right and top to bottom, writing
// palette indices rather than colours.
//
...
func (r *renderer) draw() {
	for py := 0; py < r.fb.Height; py++ {
		...
		for px := 0; px < r.fb.Width; px++ {
			...
			r.fb.SetPixel(px, py, index(escapeCount(cr, ci, r.maxIter), r.maxIter))
		}
	}
}
